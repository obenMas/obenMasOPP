...
namespace BPS
{
    public partial class frmEfficiencyCut : Form
    {
        ...
        private void fillPalletInfo()
        {
            ...
            {   
                ...
                for (int i = 0; i < lstEfficiency.Count; i++)
                {
                    ...
                    CoilP = 0;
                    CoilObs = 0;
                    for (int j = 0; j < lstEfficiency[i].coilList.Count; j++)
                    {
                        clsCoil objCoil = new clsCoil();
                        objCoil.loadSimplified(lstEfficiency[i].coilList[j]);
                        if (objCoil.fkCuttingQuality == 1)
                            CoilP += objCoil.netWeight;
                        else
                            CoilObs += objCoil.netWeight;
                    }

                    TotalConsuptionP += Math.Round(CoilP, 2, MidpointRounding.AwayFromZero);
                    TotalConsuptionObs += Math.Round(CoilObs, 2, MidpointRounding.AwayFromZero);
                    ...
                    dgvDetails.Rows[dgvDetails.Rows.Count - 1].Cells[clmconsuption.Index].Value = Math.Round(CoilP, 2, MidpointRounding.AwayFromZero);
                    dgvDetails.Rows[dgvDetails.Rows.Count - 1].Cells[clmconsuptionobs.Index].Value = Math.Round(CoilObs, 2, MidpointRounding.AwayFromZero);
