...
namespace BPS
{
    public partial class frmPalletDetailTemporaly : Form
    {
        ...
        public frmPalletDetailTemporaly()
        {
            InitializeComponent();
        }
        public frmPalletDetailTemporaly(int palletCodsec)
        {
            InitializeComponent();
        ...
        private void btnPrintCoilsLabels_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < objPallet.lstCoil.Count; i++)
            {
                Thread thrdPrint = new Thread(delegate() { printCoilLabel(objPallet.lstCoil[i].codsec); });
                thrdPrint.Start();
            }
        }
