...
namespace BPS
{
    public partial class frmFinishedProductDepot : Form
    {
        ...
        private string estadofiltrado = string.Empty;
        ...
        public frmFinishedProductDepot()
        {
            InitializeComponent();
            ObtenerDS();
            llenardgv();
        ...
        private void llenardgv()
        {
            ...
            if(DS.Tables.Count>0)
            {
                ...
                {
                    string concat = plantafiltrado + bodegafiltrado + clasificacionfiltrado + origenfiltrado + palletfiltrado + peliculafiltrado + anchofiltrado + diametrofiltrado + corefiltrado + clientefiltrado + pedidofiltrado + estadofiltrado;
                    ...
                    {
                        concat2 += Convert.ToString(DS.Tables[0].Rows[i]["Estado"]);
                    }
                    if(concat==concat2)
                    {
                        dgv.Rows.Add();
                        ...
                        if (!estados.Contains(DS.Tables[0].Rows[i]["Estado"].ToString()))
                        {
                            estados.Add(DS.Tables[0].Rows[i]["Estado"].ToString());
                        }
                    }
                }
                ...
                if (ultimocambio != "estado")
                {
                    ...
                    {
                        cmbFiltroPosicion.Items.Add(estados[i]);
                    }
                }
            }
        }
        ...
        private void cmbFiltroPosicion_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                if (estadofiltrado != cmbFiltroPosicion.SelectedItem.ToString())
                {
                    estadofiltrado = cmbFiltroPosicion.SelectedItem.ToString();
                    ultimocambio = "estado";
                    llenardgv();
                }
            }
