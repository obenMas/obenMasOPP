...
namespace BPS
{
    public partial class frmNewPreShippingControl : Form
    {
        ...
        public frmNewPreShippingControl()
        {
            InitializeComponent();
            txtObj.Text = "100";
            llenarDgv();
        ...
        public void llenarDgv()
        {
            ...
            if(DS.Tables.Count>0)
            {
                ...
                {
                    string concat = creacionfiltrado + numerofiltrado + clientefiltrado + usuariofiltrado + estadofiltrado + despachofiltrado;
        ...
        private void dgv_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if(e.ColumnIndex==editar.Index)
            {
                if(e.RowIndex!=-1)
                {
                    frmNewPreShipping winSystemConfig = new frmNewPreShipping(dgv.Rows[e.RowIndex].Cells[numero.Index].Value.ToString());
            ...
            if (e.ColumnIndex == eliminar.Index)
            {
                if (e.RowIndex != -1)
                {
                    clsNewPreShipping nps = new clsNewPreShipping(dgv.Rows[e.RowIndex].Cells[numero.Index].Value.ToString());
                    if(MessageBox.Show("¿Confirma que desea cancelar el pre-despacho","Control de pre-despachos",MessageBoxButtons.OKCancel,MessageBoxIcon.Question)==DialogResult.OK)
                    {
                        if(nps.cancel())
                        {
                            MessageBox.Show("Se cancelo el pre-despacho correctamente","Control de pre-despachos",MessageBoxButtons.OK,MessageBoxIcon.Information);
                        }
                        else
                        {
                            MessageBox.Show("No se pudo cancelar el pre-despacho seleccionado. Compruebe que el pre-despacho se encuentre abierto","Control de pre-despachos",MessageBoxButtons.OK,MessageBoxIcon.Error);
                        }
                    }
                    llenarDgv();
                }
