...
namespace BPS
{
    public partial class frmSalesOrderManagement : Form
    {
        ...
        public frmSalesOrderManagement()
        {
            InitializeComponent();
            getPalletList();
        }
        ...
        private void fillPalletInfo()
        {
            ...
            if (executeFilter == true)
            {
                ...
                for (int i = 0; i < lstSalesOrder.Count; i++)
                {
                    ...
                    bool flgOKCompromiseDate = false;
                    bool flgOKMoreThanZero = false;
                    bool flgOKMinZero = false;
                    bool flgOkIqualToZero = false;


                    if (cmbFilm.Text != "" || cmbOrder.Text != "" || cmbFilm.SelectedIndex > -1 || cmbOrder.SelectedIndex > -1 || (txtWidth.Text != "" && txtWidth.Text != "0") || cmbCustomer.Text != string.Empty || cmbCustomer.SelectedIndex > -1 || chkCompromiseDate.Checked == true || chkExtrusionDate.Checked == true || rdbAll.Checked == true || rdbMoreThanZero.Checked == true || rdbIqualToZero.Checked == true)
                    {
                        //Film
                        ...
                        {
                            if (lstSalesOrder[i].createdDate >= dtpInitDate.Value && lstSalesOrder[i].createdDate <= dtpEndDate.Value)
                        ...
                        //Compromised Date
                        if (chkCompromiseDate.Checked == true)
                        {
                            if (lstSalesOrder[i].compromiseDate >= dtpInitDateCompromise.Value && lstSalesOrder[i].compromiseDate <= dtpInitEndCompromise.Value)
                                flgOKCompromiseDate = true;
                        }
                        else
                            flgOKCompromiseDate = true;
                        ...
                        //More than Zero
                        if (rdbMoreThanZero.Checked)
                        {
                            if (lstSalesOrder[i].total > 0)
                                flgOKMoreThanZero = true;
                        }
                        else
                            flgOKMoreThanZero = true;

                        //Iqual to Zero
                        if (rdbIqualToZero.Checked)
                        {
                            if (lstSalesOrder[i].total == 0)
                                flgOkIqualToZero = true;
                        }
                        else
                            flgOkIqualToZero = true;

                        if (flgOKFilm == true && flgOKSalesOrder == true && flgOKWidth == true && flgOKCustomers == true && flgOKExtrusionDate == true && flgOKCompromiseDate == true && flgOKMoreThanZero == true && flgOKMinZero == true && flgOkIqualToZero == true)
                            flgInclude = true;
                    }
                    else
                        flgInclude = true;
                    ...
                    if (flgInclude == true)
                    {
                        ...
                        dgvDetails.Rows[dgvDetails.Rows.Count - 1].Cells[clmdateCompromise.Index].Value = " " + lstSalesOrder[i].compromiseDate.Date.ToString().Replace(" 0:00:00", "") + " ";
                        dgvDetails.Rows[dgvDetails.Rows.Count - 1].Cells[clmstatus.Index].Value = lstSalesOrder[i].statusName.ToString();
                        lastPosition = i;
                    }
                }
                if (lastPosition > -1)
        ...
        private void frmSalesOrderManagement_Load(object sender, EventArgs e)
        {
        ...
        private void btnMainCoilClearFilters_Click(object sender, EventArgs e)
        {
            ...
            cmbOrder.Text = "";
            txtWidth.Clear();
            chkCompromiseDate.Checked = false;
            chkExtrusionDate.Checked = false;
            rdbAll.Checked = false;
            rdbMoreThanZero.Checked = false;
            executeFilter = true;
            fillPalletInfo();
        }
        ...
        private void chkExtrusionDate_CheckedChanged_1(object sender, EventArgs e)
        {
            fillPalletInfo();
        }

        private void chkCompromiseDate_CheckedChanged(object sender, EventArgs e)
        {
            fillPalletInfo();
        }
        ...
        private void rdbMoreThanZero_CheckedChanged(object sender, EventArgs e)
        {
            fillPalletInfo();
        }

        private void rdbMoreThanZero_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F6)
                rdbMoreThanZero.Checked = false;
        }

        private void rdbAll_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F6)
                rdbAll.Checked = false;
        }

        private void rdbIqualToZero_CheckedChanged(object sender, EventArgs e)
        {
            fillPalletInfo();
        }

        private void rdbIqualToZero_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.F6)
                rdbIqualToZero.Checked = false;
        }
    }
}
