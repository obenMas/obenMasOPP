...
namespace BPS
{
    public partial class frmBLDetailOrigin : Form
    {
        List<clsSalesOrder> lstSalesOrder = new List<clsSalesOrder>();
        DataSet DS = new DataSet();

        public frmBLDetailOrigin()
        {
            InitializeComponent();
        ...
        {
            DS = clsConnection.getDataSetResult("SELECT * FROM vwDetalleDeBlConOrigen");
        }
        ...
        public void PopulateDGV()
        {
            ...
            for (int i = 0; i < DS.Tables[0].Rows.Count; i++)
            {
                ...
                dgvBLDetail.Rows[dgvBLDetail.Rows.Count - 1].Cells[clmDepotQuantity.Index].Value = Math.Round(Convert.ToDouble(DS.Tables[0].Rows[i]["CantidadDepositoLocal"]),2);
                dgvBLDetail.Rows[dgvBLDetail.Rows.Count - 1].Cells[clmDepotImpo.Index].Value = Math.Round(Convert.ToDouble(DS.Tables[0].Rows[i]["CantidadDepositoImpo"]), 2);
                ...
                dgvBLDetail.Rows[dgvBLDetail.Rows.Count - 1].Cells[clmQuantityDelivered.Index].Value = Math.Round(Convert.ToDouble(DS.Tables[0].Rows[i]["CantidadEntregadaLocal"]),2);
                dgvBLDetail.Rows[dgvBLDetail.Rows.Count - 1].Cells[clmEntregadoImpo.Index].Value = Math.Round(Convert.ToDouble(DS.Tables[0].Rows[i]["CantidadEntregadaImpo"]), 2);
