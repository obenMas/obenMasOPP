        ...
        public void loadCoilLayout()
        {
            ...
            for (int i = 0; i < objCuttingOrder.lstCuttingOrderDetail.Count; i++)
            {
                ...
                else
                {
                    ...
                    lstLblCoils[i].Name = "lblCoil" + i.ToString();
                    //lstLblCoils[i].Size = new System.Drawing.Size(clsCuttingOrderResume.getProportinalWidth(totalUsedWidth, Convert.ToInt32(objCuttingOrder.lstCuttingOrderDetail[i].width), 1228), 150);
                    lstLblCoils[i].Size = new System.Drawing.Size(clsCuttingOrderResume.getProportinalWidth(totalUsedWidth, Convert.ToInt32(objCuttingStop.lstCoil[pos].width), 1228), 150);
                    lstLblCoils[i].TabIndex = 0;
                    ...
                    lstLblCoilsLabels[i].Name = "lblCoilLabel" + (i + 1).ToString();
                    //lstLblCoilsLabels[i].Size = new System.Drawing.Size(clsCuttingOrderResume.getProportinalWidth(totalUsedWidth, Convert.ToInt32(objCuttingOrder.lstCuttingOrderDetail[i].width), 1228), 22);
                    lstLblCoilsLabels[i].Size = new System.Drawing.Size(clsCuttingOrderResume.getProportinalWidth(totalUsedWidth, Convert.ToInt32(objCuttingStop.lstCoil[pos].width), 1228), 22);
                    lstLblCoilsLabels[i].TabIndex = 3;
                    lstLblCoilsLabels[i].Text = "Bobina " + (i + 1).ToString();
                    lstLblCoilsLabels[i].TextAlign = System.Drawing.ContentAlignment.MiddleCenter;
                    ...
                    relativePosition += clsCuttingOrderResume.getProportinalWidth(totalUsedWidth, Convert.ToInt32(objCuttingStop.lstCoil[pos].width), 1228);
            ...
            if (validateForm())
            {
                ...
                if (lstCoil[selectedCoilIndex / 2].save(objCuttingStop.codsec, objSalesOrderDetail.codsec, ((clsCoilCellar)cmbCoilCellar.SelectedItem).codsec, objCuttingOrder.lstCuttingOrderDetail[selectedCoilIndex / 2].codsec))
                {
                    ...
                    if (MessageBox.Show("Desea imprimir la etiqueta?", "Registro de Bobina", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1, MessageBoxOptions.RightAlign) == DialogResult.Yes)
                    {
                        if (cmbQuality.Text != "Observación")
                        {
                            if (objSalesOrderDetail.sordEntityName != "CONFITECA")
                            {
                                if (objProduct.code.StartsWith("M"))
                                {
                                    Thread thrdPrint = new Thread(delegate() { clsPrintLabels.printCoilLabelM(lstCoil[selectedCoilIndex / 2].codsec, lstCoil[selectedCoilIndex / 2].externalLabelPrint); });
                                    thrdPrint.Start();
                                    thrdPrint.Join();
                                }
                                else
                                {
                                    Thread thrdPrint = new Thread(delegate() { clsPrintLabels.printCoilLabel(lstCoil[selectedCoilIndex / 2].codsec, lstCoil[selectedCoilIndex / 2].externalLabelPrint); });
                                    thrdPrint.Start();
                                    thrdPrint.Join();
                                }
                            }
                        ...
                        else
                        {
                            if (objProduct.code.StartsWith("M"))
                            {
                                Thread thrdPrint = new Thread(delegate() { clsPrintLabels.printObservationCoilLabelM(lstCoil[selectedCoilIndex / 2].codsec); });
                                thrdPrint.Start();
                                thrdPrint.Join();
                            }
                            else
                            {
                                Thread thrdPrint = new Thread(delegate() { clsPrintLabels.printObservationCoilLabel(lstCoil[selectedCoilIndex / 2].codsec); });
                                thrdPrint.Start();
                                thrdPrint.Join();
                            }
                        }
                        //clsPrintLabels.printObservationCoilLabel(lstCoil[selectedCoilIndex / 2].codsec);
