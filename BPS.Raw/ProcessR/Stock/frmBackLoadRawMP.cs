...
{
    public partial class frmBackLoadRawMP : Form
    {
        #region Init
                    ...
                    for (int i = 0; i < rawList.Count; i++)
                    {
                        ...
                        if (flgInclude == true)
                        {
                            ...
                            else
                            {
                                ...
                                dgvDetails.Rows[dgvDetails.Rows.Count - 1].Cells[clmAntiquity.Index].Value = global::BPS.Raw.Properties.Resources.clear;
                                dgvDetails.Rows[dgvDetails.Rows.Count - 1].Cells[clmDetail.Index].Value = global::BPS.Raw.Properties.Resources.clear;
                            ...
                            dgvDetails.Rows[dgvDetails.Rows.Count - 1].Cells[clmPS.Index].Value = rawList[i].code;
                            ...
                            if (ts.Days < 120)
                            {
                                dgvDetails.Rows[dgvDetails.Rows.Count - 1].Cells[clmAntiquity.Index].Value = global::BPS.Raw.Properties.Resources.bullet_green;
                                palletTotalNew += Math.Round(rawList[i].netweigth, 1);
                            }
                            else if (ts.Days >= 120 && ts.Days < 180)
                            {
                                dgvDetails.Rows[dgvDetails.Rows.Count - 1].Cells[clmAntiquity.Index].Value = global::BPS.Raw.Properties.Resources.bullet_orange;
                                palletTotalMiddle += Math.Round(rawList[i].netweigth, 1);
                            }
                            else
                            {
                                dgvDetails.Rows[dgvDetails.Rows.Count - 1].Cells[clmAntiquity.Index].Value = global::BPS.Raw.Properties.Resources.bullet_red;
                                palletTotalOld += Math.Round(rawList[i].netweigth, 1);
                            }

                            dgvDetails.Rows[dgvDetails.Rows.Count - 1].Cells[clmDetail.Index].Value = global::BPS.Raw.Properties.Resources.eye;
                            lastPosition = i;
                        }
                    }
                    if (lastPosition > -1)
        ...
        private void dgvDetails_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == clmDetail.Index)
            {
                if (Convert.ToInt32(dgvDetails.Rows[e.RowIndex].Cells[clmCodsec.Index].Value) != 0)
                {
                    ...
                    frmPalletDetailRawMP winPalletDetailTraceability = new frmPalletDetailRawMP(Convert.ToInt32(dgvDetails.Rows[e.RowIndex].Cells[clmCodsec.Index].Value));
