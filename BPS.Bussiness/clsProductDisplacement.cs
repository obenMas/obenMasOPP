...
namespace BPS.Bussiness
{
    ...
    /// </summary>
    public class clsProductDisplacement
    {
        //Atributes
        ...
        public static List<clsProductDisplacement> getProductListComplete()
        {
            List<clsProductDisplacement> lstProduct = new List<clsProductDisplacement>();
            DataSet DS = new DataSet();

            DS = clsConnection.getDataSetResult("SELECT COALESCE(prod_codsec,0) AS prod_codsec,COALESCE(prod_fkProductType,0) AS prod_fkProductType,COALESCE(prod_fkBopp,0) AS prod_fkBopp,COALESCE(prod_fkCast,0) AS prod_fkCast,COALESCE(prod_fkCore,0) AS prod_fkCore,COALESCE(prod_fkDiameter,0) AS prod_fkDiameter,COALESCE(prod_fkStatus,0) AS prod_fkStatus,COALESCE(prod_code,N'') AS prod_code,COALESCE(prod_width,0) AS prod_width FROM dbo.bps_prod_product ORDER BY bps_prod_product.prod_code ASC");

            for (int i = 0; i < DS.Tables[0].Rows.Count; i++)
            {
                lstProduct.Add(new clsProductDisplacement());
        ...
        //public clsProductDisplacement(int codsec)
        //{
        //    this.load(codsec);
        //}
        //public clsProductDisplacement(int codsec, bool simple)
        //{
        //    this.load(codsec, true);
        //}
        //public clsProductDisplacement(string code)
        //{
        //    this.load(code);
        //}
        ////Methods
        ...
        //    DS = clsConnection.getDataSetResult("SELECT * FROM vwProduct WHERE prod_fkCast > 0 " + where + " ORDER BY prod_name");
        ...
        //internal static List<clsProduct> getProductListInStock()
        //{
        //    List<clsProduct> lstProduct = new List<clsProduct>();
        //    DataSet DS = new DataSet();
