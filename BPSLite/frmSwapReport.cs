        ...
        private void btnGenerateCuttingRp_Click(object sender, EventArgs e)
        {
            ...
            if(DS.Tables.Count>0 && DS.Tables[0].Rows.Count>0)
            {
                for(int i=0;i<DS.Tables[0].Rows.Count;i++)
                {
                    ...
                    if (chkBobina.Checked)
                    {
                        if(Convert.ToBoolean(DS.Tables[0].Rows[i]["esExterna"]))
                        {
                            dgvSwapInterplant.Rows[dgvSwapInterplant.Rows.Count - 1].Cells[clmCoilType.Index].Value = "IMPORTADA";
                        }
                        else
                        {
                            dgvSwapInterplant.Rows[dgvSwapInterplant.Rows.Count - 1].Cells[clmCoilType.Index].Value = "LOCAL";
                        }
