...
namespace BPS
{
    public partial class frmDepotState : Form
    {
        ...
        public frmDepotState(string planta)
        {
            InitializeComponent();
            getDataSet();
            populateDataGrid();

            for(int i = 0; i<cmbPlant.Items.Count;i++)
            {
                if(planta == cmbPlant.Items[i].ToString())
                {
                    cmbPlant.SelectedIndex = i;
                }
            }
            ...
            if (DS.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < DS.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (concat == concat2)
                    {
                        ...
                        //Agregado para arreglar las fechas
                        dgvDepotState.Rows[dgvDepotState.Rows.Count - 1].Cells["clmDayCorte"].Value = DS.Tables[0].Rows[i]["dayCorte"];
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            dgvDepotState.Columns["clmDayCorte"].Visible = true;
            ...
            dgvDepotState.Columns["clmDayCorte"].Visible = false;
