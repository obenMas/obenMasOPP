...
namespace BPS.Lite
{
    public partial class frmCoilHistory : Form
    {
        public frmCoilHistory()
        {
            InitializeComponent();
        ...
        public void populateDatagrid()
        {
            ...
            {
                DS = DeterminarTurno("SELECT * from bps_prod_depotcoilhistory WHERE dh_day=" + dtpFecha.Value.Day.ToString() + "AND dh_month=" + dtpFecha.Value.Month.ToString() + "AND dh_year=" + dtpFecha.Value.Year.ToString());
            }
            else if (cmbPlanta.Items[cmbPlanta.SelectedIndex].ToString() == "Pilar")
            {
                DS = DeterminarTurno("SELECT * from bps_prod_depotcoilhistory WHERE dh_day=" + dtpFecha.Value.Day.ToString() + "AND dh_month=" + dtpFecha.Value.Month.ToString() + "AND dh_year=" + dtpFecha.Value.Year.ToString() + "AND dh_plant=4");
            }
            else if (cmbPlanta.Items[cmbPlanta.SelectedIndex].ToString() == "Campana")
            {
                DS = DeterminarTurno("SELECT * from bps_prod_depotcoilhistory WHERE dh_day=" + dtpFecha.Value.Day.ToString() + "AND dh_month=" + dtpFecha.Value.Month.ToString() + "AND dh_year=" + dtpFecha.Value.Year.ToString() + "AND dh_plant=3");
            }
            ...
            if (DS.Tables.Count > 0 && DS.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < DS.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (DS.Tables[0].Rows[i]["dh_external"]!=DBNull.Value)
                    {
                        switch (DS.Tables[0].Rows[i]["dh_external"].ToString())
                        {
                            case "1": dataGridView1.Rows[dataGridView1.Rows.Count - 1].Cells[isexternal.Index].Value = "IMPORTADO";
                                break;
                            case "0": dataGridView1.Rows[dataGridView1.Rows.Count - 1].Cells[isexternal.Index].Value = "LOCAL";
                                break;
                        }
                    }
                    else
                    {
                        dataGridView1.Rows[dataGridView1.Rows.Count - 1].Cells[isexternal.Index].Value = "SIN DATOS";
                    }
