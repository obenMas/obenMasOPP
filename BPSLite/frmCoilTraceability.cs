    ...
    public partial class frmCoilTraceability : Form
    {
        ...
        private void getCoilInformation(string Code)
        {
            ...
            if (objCoil.codsec != 0)
            {
                ...
                lblNetWeigth.Text = calcularPesoNeto(objCoil).ToString();
                //lblNetWeigth.Text = Convert.ToString(Math.Round(objCoil.netWeight, 2)); Lo mismo para esto, se saco por que el peso neto se calcula a partir del metraje. 
                flgIsChangingNewWeigth = true;
                lblGrossWeigth.Text = Convert.ToString(Math.Round(objCoil.grossWeight, 2));
                flgIsChangingNewWeigth = false;
                //lblLength.Text = Convert.ToString(Math.Round(objCoil.length, 2)); Esto se saco ya que el neto de metraje es otro actualmente, por que hay consumos.
                lblLength.Text = (Math.Round(objCoil.length, 2) - objCoil.getUsedLength(objCoil.codsec)).ToString();
                lblBalance.Text = objCoil.BalanceName;
                lblNotes.Text = objCoil.notes;
                lblWidth.Text = objCoil.width.ToString();
                lblEstado.Text = objCoil.statusName;
                ...
                if (objCuttingStop.lstMainCoilByCuttingStop.Count > 0)
                {
                    ...
                    for (int i = 0; i < objCuttingStop.lstMainCoilByCuttingStop.Count; i++)
                    {
                        ...
                        dgvMainCoilList.Rows[i].Cells[clmMainCoilOutline.Index].Value = global::BPS.Lite.Properties.Resources.chart_line;
                        dgvMainCoilList.Rows[i].Cells[clmMainCoilQuality.Index].Value = global::BPS.Lite.Properties.Resources.report;
                    }
                }
                else if (objCuttingStop.lstSecundaryCutCoil.Count > 0)
                {
                    ...
                    for (int i = 0; i < objCuttingStop.lstSecundaryCutCoil.Count; i++)
                    {
                        ...
                        dgvCoilsCutted.Rows[i].Cells[clmMainCoilOutline.Index].Value = global::BPS.Lite.Properties.Resources.coils;
                    }
                }
                else
                {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Desea imprimir la etiqueta?", "Listado de Bobinas por orden de venta", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1, MessageBoxOptions.RightAlign) == DialogResult.Yes)
            {
                ...
                //{
                    if (objCoil.customerName == "ENVASANDO S.R.L.")
                    {
                        Thread thrdPrint = new Thread(delegate() { BPS.Lite.clsPrintLabels.printCoilLabelSpecial(objCoil.codsec, objCoil.externalLabelPrint); });
                        thrdPrint.Start();
                        thrdPrint.Join();
                    ...
                    {
                        switch(objCoil.coilCellarContainer.fkPlant)
                        {
                            case 3:
                                Thread thrdPrint = new Thread(delegate() { BPS.Lite.clsPrintLabels.printCoilLabel(objCoil.codsec, objCoil.externalLabelPrint); });
                                thrdPrint.Start();
                                thrdPrint.Join();
                                break;
                            case 4:
                                Thread thrdPrintPL = new Thread(delegate() { BPS.Lite.clsPrintLabels.printCoilLabelPL(objCoil.codsec, objCoil.externalLabelPrint); });
                                thrdPrintPL.Start();
                                thrdPrintPL.Join();
                                break;
                            default:
                                Thread thrdPrint2 = new Thread(delegate() { BPS.Lite.clsPrintLabels.printCoilLabel(objCoil.codsec, objCoil.externalLabelPrint); });
                                thrdPrint2.Start();
                                thrdPrint2.Join();
                                break;
                        }
                ...
                /*{
                    ...
                    {
                        Thread thrdPrint = new Thread(delegate() { BPS.Lite.clsPrintLabels.printObservationCoilLabelM(objCoil.codsec); });
                        thrdPrint.Start();
                        thrdPrint.Join();
                    }
                    else
                    {
                        Thread thrdPrint = new Thread(delegate() { BPS.Lite.clsPrintLabels.printObservationCoilLabel(objCoil.codsec); });
                        thrdPrint.Start();
                        thrdPrint.Join();
                    }
                }*/
        ...
        private double calcularPesoNeto(object coil)
        {
