...
namespace BPS.Lite
{
    public partial class frmCompletePalletInventory : Form
    {
        ...
        public frmCompletePalletInventory()
        {
            InitializeComponent();
            ObtenerDS();
            llenardgv();
        ...
        private void llenardgv()
        {
            ...
            if(DS.Tables.Count>0)
            {
                ...
                {
                    string concat = plantafiltrado + bodegafiltrado + clasificacionfiltrado + origenfiltrado + palletfiltrado + peliculafiltrado + anchofiltrado + diametrofiltrado + corefiltrado + clientefiltrado + pedidofiltrado + posicionfiltrado;
                    ...
                    if(concat==concat2)
                    {
                        ...
                        dgv.Rows[dgv.Rows.Count - 1].Cells[PesoNeto.Index].Value = Math.Round(Convert.ToDouble(DS.Tables[0].Rows[i]["PesoNeto"]),2);
                        ...
                        dgv.Rows[dgv.Rows.Count - 1].Cells[PD.Index].Value = DS.Tables[0].Rows[i]["PD"];
                        dgv.Rows[dgv.Rows.Count - 1].Cells[estadoPD.Index].Value = DS.Tables[0].Rows[i]["estadoPred"];
                        if (DS.Tables[0].Rows[i]["PD"].ToString() != "-")
                        {
                            dgv.Rows[dgv.Rows.Count - 1].Cells[fechaProg.Index].Value = DS.Tables[0].Rows[i]["fechaDesp"];
                        }
            ...
            txtTotPallets.Text = Convert.ToString(dgv.Rows.Count);
            txtTotKg.Text = Convert.ToString(Math.Round(pesos, 2));
            txtTotBobinas.Text = cantBob.ToString();
        ...
        private void cmbFiltroClasificacion_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private void cmbFiltroClasificacion_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Back)
            {
