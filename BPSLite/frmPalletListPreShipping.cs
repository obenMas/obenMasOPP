...
namespace BPS.Lite
{
    public partial class frmPalletListPreShipping : Form
    {
        ...
        public frmPalletListPreShipping(int planta)
        {
            InitializeComponent();
            fkplant = planta;
        }
        ...
        private void FillDataPreShipping()
        {
            ...
            for (int i = 0; i < lstPalletPreShipping.Count; i++)
            {
                ...
                dgvPalletPreShipping.Rows[i].Cells[clmview.Index].Value = global::BPS.Lite.Properties.Resources.eye;
                dgvPalletPreShipping.Rows[i].Cells[clmreport.Index].Value = global::BPS.Lite.Properties.Resources.report;
                dgvPalletPreShipping.Rows[i].Cells[clmisinternational.Index].Value = global::BPS.Lite.Properties.Resources.delivery;
        ...
        private void dgvPalletPreShipping_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (e.ColumnIndex == clmreport.Index)
            {
                ...
                {
                    frmPalletPreShippingReportPL winPallet = new frmPalletPreShippingReportPL(dgvPalletPreShipping.Rows[e.RowIndex].Cells[clmparameter.Index].Value.ToString());
                    winPallet.StartPosition = FormStartPosition.CenterScreen;
                    winPallet.MdiParent = this.MdiParent;
                    winPallet.Show();
                }
            ...
            else if (e.ColumnIndex == clmisinternational.Index)
            {
                ...
                {
                    frmPackingListReportPL winPallet = new frmPackingListReportPL(dgvPalletPreShipping.Rows[e.RowIndex].Cells[clmnumber.Index].Value.ToString());
                    winPallet.StartPosition = FormStartPosition.CenterScreen;
                    winPallet.MdiParent = this.MdiParent;
                    winPallet.Show();
                }
